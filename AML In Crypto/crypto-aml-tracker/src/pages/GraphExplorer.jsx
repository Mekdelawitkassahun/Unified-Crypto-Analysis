...
const AddressPanel = ({ address, stats, edgesData, onClose }) => {
  ...
  return (
    <div style={{
      ...
      background: '#0d1728', borderLeft: '1px solid rgba(201,168,76,0.15)',
      boxShadow: '-4px 0 40px rgba(0,0,0,0.6),inset 1px 0 0 rgba(201,168,76,0.1)',
      display: 'flex', flexDirection: 'column', zIndex: 10,
      overflowY: 'auto',
    }}>
      <div style={{ padding: '16px 18px', borderBottom: '1px solid rgba(201,168,76,0.12)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          <div style={{ fontSize: '12px', color: '#6b7a8d', marginBottom: '4px' }}>
            {stats.isHighValue ? 'High-value address' : 'Address'}
          </div>
          <div style={{ fontSize: '11px', fontFamily: 'monospace', color: '#edf2f8', wordBreak: 'break-all' }}>
            {address}
          </div>
        </div>
        <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#4a5568', fontSize: '18px', marginLeft: '8px', flexShrink: 0 }}>✕</button>
      </div>

      {/* Owner section */}
      <div style={{ padding: '12px 18px', borderBottom: '1px solid rgba(201,168,76,0.08)' }}>
        <div style={{ fontSize: '11px', fontWeight: '600', color: '#8f9bad', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '8px' }}>
          Owner
        </div>
        {renderOwnerSection()}
      </div>
      ...
      <div style={{ padding: '14px 18px', borderBottom: '1px solid rgba(201,168,76,0.08)' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
          {[
            ...
            <div key={label} style={{ background: 'rgba(22,33,52,0.95)', borderRadius: '8px', padding: '10px', border: '1px solid rgba(201,168,76,0.15)' }}>
              <div style={{ fontSize: '10px', color: '#4a5568', marginBottom: '3px' }}>{label}</div>
              <div style={{ fontSize: '13px', fontWeight: '700', color: '#edf2f8' }}>{value}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ padding: '12px 18px', borderBottom: '1px solid rgba(201,168,76,0.08)', display: 'flex', gap: '8px' }}>
        <a href={`https://etherscan.io/address/${address}`} target="_blank" rel="noreferrer"
          style={{ flex: 1, textAlign: 'center', padding: '8px', background: 'rgba(29,78,216,0.15)', color: '#60a5fa', borderRadius: '7px', fontSize: '12px', fontWeight: '600', textDecoration: 'none', border: '1px solid rgba(29,78,216,0.3)' }}>
          🔍 Etherscan
        </a>
        <a href={`https://etherscan.io/address/${address}#tokentxns`} target="_blank" rel="noreferrer"
          style={{ flex: 1, textAlign: 'center', padding: '8px', background: 'rgba(22,101,52,0.15)', color: '#4ade80', borderRadius: '7px', fontSize: '12px', fontWeight: '600', textDecoration: 'none', border: '1px solid rgba(22,101,52,0.3)' }}>
          🪙 Tokens
        </a>
      </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 48px)', gap: '12px', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
        <div>
          <div style={{ fontSize: '18px', fontWeight: '700', color: '#edf2f8' }}>Transaction Network</div>
        ...
        <form onSubmit={handleSearch} style={{ display: 'flex', gap: '8px', flex: 1, maxWidth: '480px' }}>
          <input
            ...
            style={{
              flex: 1, padding: '8px 14px', border: '1px solid rgba(201,168,76,0.2)',
              borderRadius: '8px', fontSize: '13px', outline: 'none', background: 'rgba(15,24,38,0.9)', color: '#edf2f8',
            }}
          />
          ...
          {search && (
            <button type="button" onClick={() => {
              ...
              padding: '8px 12px', background: 'rgba(22,33,52,0.9)', color: '#8f9bad',
              border: '1px solid rgba(201,168,76,0.2)', borderRadius: '8px', cursor: 'pointer', fontSize: '12px',
            }}>
              Clear
            </button>
          )}
      ...
      {/* ── Graph canvas ── */}
      <div style={{ flex: 1, position: 'relative', borderRadius: '12px', overflow: 'hidden', border: '1px solid rgba(201,168,76,0.15)', background: '#0a1020', minHeight: 0 }}>
        {loading
            ...
            : (
              ...
              >
                ...
                <MiniMap
                  ...
                  style={{ background: '#0d1728', border: '1px solid rgba(201,168,76,0.15)', borderRadius: '8px' }}
                />
              </ReactFlow>
            )
        }
