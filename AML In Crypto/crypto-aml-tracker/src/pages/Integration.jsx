...
const SIGNAL_META = {
    convergence: { label: 'Convergence', icon: '🔀', color: '#a78bfa', bg: 'rgba(124,58,237,0.2)', border: 'rgba(124,58,237,0.35)' },
    dormancy: { label: 'Dormancy', icon: '💤', color: '#38bdf8', bg: 'rgba(3,105,161,0.2)', border: 'rgba(3,105,161,0.35)' },
    terminal_node: { label: 'Terminal Node', icon: '🚪', color: '#f87171', bg: 'rgba(185,28,28,0.2)', border: 'rgba(185,28,28,0.35)' },
    reaggregation: { label: 'Reaggregation', icon: '🔁', color: '#fbbf24', bg: 'rgba(217,119,6,0.2)', border: 'rgba(217,119,6,0.35)' },
};
...
const scoreColor = (score) => {
    if (score >= 0.70) return { text: '#f87171', bg: 'rgba(185,28,28,0.15)', border: 'rgba(185,28,28,0.3)' };
    if (score >= 0.50) return { text: '#fbbf24', bg: 'rgba(217,119,6,0.15)', border: 'rgba(217,119,6,0.3)' };
    return { text: '#4ade80', bg: 'rgba(22,163,74,0.15)', border: 'rgba(22,163,74,0.3)' };
};
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '14px' }}>
                {[
                    ...
                    const c = {
                        danger: { bg: 'rgba(185,28,28,0.15)', border: 'rgba(185,28,28,0.3)', text: '#f87171', num: '#ef4444' },
                        warning: { bg: 'rgba(217,119,6,0.15)', border: 'rgba(217,119,6,0.3)', text: '#fbbf24', num: '#f59e0b' },
                        accent: { bg: 'rgba(2,132,199,0.15)', border: 'rgba(2,132,199,0.3)', text: '#38bdf8', num: '#0ea5e9' },
                        purple: { bg: 'rgba(124,58,237,0.15)', border: 'rgba(124,58,237,0.3)', text: '#a78bfa', num: '#8b5cf6' },
                        blue: { bg: 'rgba(29,78,216,0.15)', border: 'rgba(29,78,216,0.3)', text: '#60a5fa', num: '#3b82f6' },
                    }[tone];
                    return (
            ...
            <div style={{ background: 'rgba(22,33,52,0.95)', border: '1px solid rgba(201,168,76,0.18)', borderRadius: '14px', padding: '14px 16px', display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center' }}>
                <div style={{ position: 'relative', flex: 1, minWidth: '220px' }}>
                    ...
                    <input
                        ...
                        style={{ width: '100%', paddingLeft: '34px', paddingRight: '12px', paddingTop: '9px', paddingBottom: '9px', borderRadius: '10px', border: '1px solid rgba(201,168,76,0.2)', fontSize: '13px', background: 'rgba(15,24,38,0.9)', color: '#edf2f8', outline: 'none', boxSizing: 'border-box' }}
                    />
                </div>
            ...
            <div style={{ background: 'rgba(22,33,52,0.95)', border: '1px solid rgba(201,168,76,0.18)', borderRadius: '14px', padding: '14px 16px', display: 'flex', gap: '16px', flexWrap: 'wrap', alignItems: 'flex-start' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    <div style={{ fontSize: '11px', fontWeight: '700', color: '#8f9bad', textTransform: 'uppercase', letterSpacing: '0.1em' }}>📅 Filter by Analysis Date</div>
                    <input
                ...
                {runs.length > 0 && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', flex: 1 }}>
                        <div style={{ fontSize: '11px', fontWeight: '700', color: '#8f9bad', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Available Runs</div>
                        <select
                            value={dateTimeInput}
                            onChange={(e) => {
            ...
            <div style={{ background: 'rgba(22,33,52,0.95)', border: '1px solid rgba(201,168,76,0.18)', borderRadius: '16px', overflow: 'hidden' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '2fr 1.2fr 1.5fr 1fr', padding: '10px 20px', background: 'rgba(18,28,44,0.95)', borderBottom: '2px solid rgba(201,168,76,0.2)' }}>
                    {['Entity', 'Signals Fired', 'Reason', 'Score'].map((h) => (
                        <div key={h} style={{ fontSize: '11px', fontWeight: '800', color: '#8f9bad', textTransform: 'uppercase', letterSpacing: '0.1em' }}>{h}</div>
                    ))}
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 20px', borderBottom: '1px solid rgba(201,168,76,0.08)', background: 'rgba(15,24,38,0.85)' }}>
                    <span style={{ fontSize: '12px', color: '#4a5568' }}>{filteredAlerts.length} alerts — page {page} of {totalPages}</span>
                </div>

                {visibleAlerts.length === 0 ? (
                    <div style={{ padding: '48px 20px', textAlign: 'center', color: '#4a5568', fontSize: '13px' }}>
                        {alerts.length === 0
                    ...
                    return (
                        <div
                            key={alert.entity_id}
                            style={{ display: 'grid', gridTemplateColumns: '2fr 1.2fr 1.5fr 1fr', padding: '14px 20px', borderBottom: idx < visibleAlerts.length - 1 ? '1px solid rgba(201,168,76,0.08)' : 'none', background: idx % 2 === 0 ? 'rgba(22,33,52,0.6)' : 'rgba(15,24,38,0.5)', borderLeft: '3px solid transparent', alignItems: 'center' }}
                            onMouseEnter={(e) => { e.currentTarget.style.background = 'rgba(201,168,76,0.08)'; e.currentTarget.style.borderLeft = '3px solid rgba(201,168,76,0.4)'; }}
                        ...
                        >
                            ...
                            <div style={{ minWidth: 0 }}>
                                <div style={{ fontSize: '10px', fontWeight: '700', color: '#4a5568', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '3px' }}>📍 Address</div>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                ...
                                <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
                                    ...
                                    <button
                                        ...
                                        style={{ fontSize: '10px', padding: '1px 7px', borderRadius: '999px', background: 'rgba(201,168,76,0.12)', color: '#c9a84c', border: '1px solid rgba(201,168,76,0.25)', cursor: 'pointer', fontWeight: '700' }}
                                    >
                                        🔗 Chain
                                    </button>
                                </div>
                ...
                {totalPages > 1 && (
                    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px', padding: '14px 20px', borderTop: '1px solid rgba(201,168,76,0.08)' }}>
                        <button onClick={() => setPage(1)} disabled={page === 1} style={{ padding: '5px 10px', borderRadius: '6px', border: '1px solid rgba(201,168,76,0.2)', background: page === 1 ? 'rgba(15,24,38,0.5)' : 'rgba(22,33,52,0.9)', color: page === 1 ? 'rgba(201,168,76,0.25)' : '#c9a84c', cursor: page === 1 ? 'not-allowed' : 'pointer', fontSize: '12px' }}>«</button>
                        <button onClick={() => setPage(p => p - 1)} disabled={page === 1} style={{ padding: '5px 10px', borderRadius: '6px', border: '1px solid rgba(201,168,76,0.2)', background: page === 1 ? 'rgba(15,24,38,0.5)' : 'rgba(22,33,52,0.9)', color: page === 1 ? 'rgba(201,168,76,0.25)' : '#c9a84c', cursor: page === 1 ? 'not-allowed' : 'pointer', fontSize: '12px' }}>‹</button>
                        <span style={{ fontSize: '12px', color: '#6b7a8d', padding: '0 8px' }}>Page {page} / {totalPages}</span>
                        <button onClick={() => setPage(p => p + 1)} disabled={page === totalPages} style={{ padding: '5px 10px', borderRadius: '6px', border: '1px solid rgba(201,168,76,0.2)', background: page === totalPages ? 'rgba(15,24,38,0.5)' : 'rgba(22,33,52,0.9)', color: page === totalPages ? 'rgba(201,168,76,0.25)' : '#c9a84c', cursor: page === totalPages ? 'not-allowed' : 'pointer', fontSize: '12px' }}>›</button>
                        <button onClick={() => setPage(totalPages)} disabled={page === totalPages} style={{ padding: '5px 10px', borderRadius: '6px', border: '1px solid rgba(201,168,76,0.2)', background: page === totalPages ? 'rgba(15,24,38,0.5)' : 'rgba(22,33,52,0.9)', color: page === totalPages ? 'rgba(201,168,76,0.25)' : '#c9a84c', cursor: page === totalPages ? 'not-allowed' : 'pointer', fontSize: '12px' }}>»</button>
                    </div>
                )}
            </div>
            {custodyEntity && (
