...
export default function Placement({ onNavigateToGraph }) {
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            {/* Header */}
            <div style={{ background: 'linear-gradient(135deg, #0d1b2e 0%, #0f2744 100%)', borderRadius: '16px', padding: '28px 32px', color: '#fff', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '16px', border: '1px solid rgba(201,168,76,0.2)', boxShadow: '0 4px 24px rgba(0,0,0,0.4),inset 0 1px 0 rgba(201,168,76,0.08)' }}>
                <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '14px' }}>
                {[
                    ...
                    const c = { danger: { bg: 'rgba(185,28,28,0.15)', border: 'rgba(185,28,28,0.3)', text: '#f87171', num: '#ef4444' }, warning: { bg: 'rgba(217,119,6,0.15)', border: 'rgba(217,119,6,0.3)', text: '#fbbf24', num: '#f59e0b' }, accent: { bg: 'rgba(2,132,199,0.15)', border: 'rgba(2,132,199,0.3)', text: '#38bdf8', num: '#0ea5e9' } }[tone];
                    return (
            ...
            {/* Search + filter */}
            <div style={{ background: 'rgba(22,33,52,0.95)', border: '1px solid rgba(201,168,76,0.18)', borderRadius: '14px', padding: '14px 16px', display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center' }}>
                <div style={{ position: 'relative', flex: 1, minWidth: '220px' }}>
                    <span style={{ position: 'absolute', left: '12px', top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none' }}>🔎</span>
                    <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search entity or address..." style={{ width: '100%', paddingLeft: '34px', paddingRight: '12px', paddingTop: '9px', paddingBottom: '9px', borderRadius: '10px', border: '1px solid rgba(201,168,76,0.2)', fontSize: '13px', background: 'rgba(15,24,38,0.9)', color: '#edf2f8', outline: 'none', boxSizing: 'border-box' }} />
                </div>
                <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                    <select
                        value={behaviorFilter === 'All' ? '' : behaviorFilter}
                        onChange={(e) => setBehaviorFilter(e.target.value || 'All')}
                        style={{ padding: '8px 12px', borderRadius: '10px', border: '1px solid rgba(201,168,76,0.2)', background: 'rgba(15,24,38,0.9)', color: '#edf2f8', fontSize: '13px', fontWeight: '600', cursor: 'pointer', outline: 'none' }}
                    >
            ...
            <div style={{ background: 'rgba(22,33,52,0.95)', border: '1px solid rgba(201,168,76,0.18)', borderRadius: '14px', padding: '14px 16px', display: 'flex', gap: '16px', flexWrap: 'wrap', alignItems: 'flex-start' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    <div style={{ fontSize: '11px', fontWeight: '700', color: '#6b7a8d', textTransform: 'uppercase', letterSpacing: '0.1em' }}>📅 Filter by Analysis Date</div>
                    <input type="datetime-local" value={dateTimeInput} max={new Date().toISOString().slice(0, 16)} onChange={(e) => { setDateTimeInput(e.target.value); setSelectedRunId(null); }} style={{ padding: '8px 12px', borderRadius: '10px', border: '1px solid rgba(201,168,76,0.2)', background: 'rgba(15,24,38,0.9)', color: '#edf2f8', fontSize: '13px', fontWeight: '600', cursor: 'pointer', outline: 'none' }} />
                    <div style={{ fontSize: '11px', color: '#4a5568' }}>Shows the run closest to the selected date</div>
                </div>
                {runs.length > 0 && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', flex: 1 }}>
                        <div style={{ fontSize: '11px', fontWeight: '700', color: '#6b7a8d', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Available Runs</div>
                        <select
                            value={dateTimeInput}
                            onChange={(e) => {
                            ...
                            style={{ padding: '8px 12px', borderRadius: '10px', border: '1px solid rgba(201,168,76,0.2)', background: 'rgba(15,24,38,0.9)', color: '#edf2f8', fontSize: '13px', fontWeight: '600', cursor: 'pointer', outline: 'none', maxWidth: '320px' }}
                        >
                            {runs.map((run, i) => {
            ...
            {/* Alert table */}
            <div style={{ background: 'rgba(22,33,52,0.95)', border: '1px solid rgba(201,168,76,0.18)', borderRadius: '16px', overflow: 'hidden' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', padding: '10px 20px', background: 'rgba(18,28,44,0.95)', borderBottom: '1px solid rgba(201,168,76,0.12)' }}>
                    {['Entity', 'Behaviors', 'Reason', 'Confidence'].map((h) => (
                        <div key={h} style={{ fontSize: '11px', fontWeight: '800', color: '#6b7a8d', textTransform: 'uppercase', letterSpacing: '0.1em' }}>{h}</div>
                    ))}
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 20px', borderBottom: '1px solid rgba(201,168,76,0.08)', background: 'rgba(13,21,34,0.85)' }}>
                    <span style={{ fontSize: '12px', color: '#4a5568' }}>{filteredAlerts.length} alerts — page {page} of {totalPages}</span>
                </div>
                {visibleAlerts.length === 0 ? (
                    <div style={{ padding: '48px 20px', textAlign: 'center', color: '#4a5568', fontSize: '13px' }}>No alerts match the current filters.</div>
                    ...
                    return (
                        <div key={alert.entity_id} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', padding: '14px 20px', borderBottom: idx < visibleAlerts.length - 1 ? '1px solid rgba(201,168,76,0.08)' : 'none', background: idx % 2 === 0 ? 'rgba(22,33,52,0.6)' : 'rgba(15,24,38,0.5)', borderLeft: '3px solid transparent', alignItems: 'center' }}
                            onMouseEnter={(e) => { e.currentTarget.style.background = 'rgba(201,168,76,0.08)'; e.currentTarget.style.borderLeft = '3px solid #0f6578'; }}
                            onMouseLeave={(e) => { e.currentTarget.style.background = idx % 2 === 0 ? 'rgba(22,33,52,0.6)' : 'rgba(15,24,38,0.5)'; e.currentTarget.style.borderLeft = '3px solid transparent'; }}
                        >
                            {/* Entity */}
                            <div style={{ minWidth: 0 }}>
                                <div style={{ fontSize: '10px', fontWeight: '700', color: '#4a5568', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '3px' }}>{isCluster ? '🔗 Cluster' : '📍 Address'}</div>
                                <div style={{ fontSize: '13px', fontWeight: '700', color: alert.entity_name ? '#edf2f8' : '#4a5568', marginBottom: '2px', fontStyle: alert.entity_name ? 'normal' : 'italic' }}>
                                    {alert.entity_name || 'Unknown'}
                                </div>
                                {isCluster ? (
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                                        <span style={{ fontSize: '11px', fontFamily: 'monospace', color: '#6b7a8d' }}>{truncate(alert.entity_id, 24)}</span>
                                ...
                                <div style={{ fontSize: '11px', color: '#4a5568', marginTop: '2px' }}>{alert.address_count} addresses</div>
                            </div>
                                ...
                                {(() => {
                                    ...
                                    return sorted.map((bType, i) => {
                                        ...
                                        const style = isDominant
                                            ? { bg: t === 'danger' ? 'rgba(185,28,28,0.2)' : t === 'warning' ? 'rgba(217,119,6,0.2)' : 'rgba(3,105,161,0.2)', color: t === 'danger' ? '#f87171' : t === 'warning' ? '#fbbf24' : '#38bdf8', border: t === 'danger' ? 'rgba(185,28,28,0.35)' : t === 'warning' ? 'rgba(217,119,6,0.35)' : 'rgba(3,105,161,0.35)', fontWeight: '800' }
                                            : isSecondary
                                                ? { bg: 'rgba(22,33,52,0.95)', color: '#8f9bad', border: 'rgba(201,168,76,0.2)', fontWeight: '600' }
                                                : { bg: 'rgba(15,24,38,0.9)', color: '#4a5568', border: 'rgba(201,168,76,0.12)', fontWeight: '500' };
                                        return (
                            ...
                            {/* Reason */}
                            <div style={{ fontSize: '11px', color: '#6b7a8d', lineHeight: 1.5, paddingRight: '12px' }}>
                                {(() => {
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                <div style={{ flex: 1, height: '6px', background: 'rgba(28,41,64,0.8)', borderRadius: '3px', overflow: 'hidden' }}>
                                    <div style={{ width: `${Math.round((alert.confidence || 0) * 100)}%`, height: '100%', background: '#0f6578', borderRadius: '3px' }} />
                                </div>
                                <span style={{ fontSize: '11px', fontWeight: '700', color: '#8f9bad', minWidth: '32px' }}>{formatNumber((alert.confidence || 0) * 100, 0)}%</span>
                            </div>
                        </div>
                    );
                })}
                ...
                {/* Pagination */}
                {totalPages > 1 && (
                    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px', padding: '14px 20px', borderTop: '1px solid rgba(201,168,76,0.08)', background: 'rgba(13,21,34,0.85)' }}>
                        {[
                            ...
                            btn === null ? (
                                <span key="counter" style={{ fontSize: '12px', color: '#6b7a8d', minWidth: '70px', textAlign: 'center' }}>{page} / {totalPages}</span>
                            ) : (
                                <button key={btn.label} type="button" onClick={btn.action} disabled={btn.disabled} style={{ padding: '6px 14px', borderRadius: '8px', border: '1px solid rgba(201,168,76,0.2)', background: btn.disabled ? 'rgba(15,24,38,0.5)' : 'rgba(22,33,52,0.9)', color: btn.disabled ? 'rgba(201,168,76,0.25)' : '#c9a84c', fontSize: '12px', fontWeight: '700', cursor: btn.disabled ? 'not-allowed' : 'pointer' }}>
                                    {btn.label}
                                </button>
                            )
                        )}
            ...
            {clusterPopup && (
                <>
                    ...
                    <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: '380px', background: '#0d1728', zIndex: 1000, boxShadow: '-4px 0 40px rgba(0,0,0,0.6),inset 1px 0 0 rgba(201,168,76,0.1)', display: 'flex', flexDirection: 'column' }}>
                        <div style={{ padding: '20px 24px', borderBottom: '1px solid rgba(201,168,76,0.12)', background: 'linear-gradient(135deg,rgba(13,23,42,0.98),rgba(18,32,56,0.98))' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                <div>
                                    ...
                                    <div style={{ fontSize: '12px', fontFamily: 'monospace', color: '#edf2f8', wordBreak: 'break-all' }}>{clusterPopup.entityId}</div>
                                </div>
                                <button type="button" onClick={() => setClusterPopup(null)} style={{ border: '1px solid rgba(201,168,76,0.2)', background: 'rgba(22,33,52,0.9)', borderRadius: '8px', padding: '4px 10px', cursor: 'pointer', fontSize: '14px', color: '#8f9bad', flexShrink: 0, marginLeft: '12px' }}>✕</button>
                            </div>
                            <div style={{ marginTop: '10px', fontSize: '12px', color: '#6b7a8d' }}>{clusterPopup.addresses.length} addresses</div>
                        </div>
                        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 24px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                            {clusterPopup.addresses.length === 0 ? (
                                <div style={{ color: '#4a5568', fontSize: '13px', textAlign: 'center', padding: '40px 0' }}>No addresses available</div>
                            ) : clusterPopup.addresses.map((addr) => (
                                <div key={addr} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 14px', borderRadius: '10px', background: 'rgba(22,33,52,0.95)', border: '1px solid rgba(201,168,76,0.18)', gap: '8px' }}>
                                    <span style={{ fontFamily: 'monospace', fontSize: '11px', color: '#edf2f8', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1 }} title={addr}>{addr}</span>
            ...
            {reasonPopup && (
                <>
                    ...
                    <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: '420px', background: '#0d1728', zIndex: 1000, boxShadow: '-4px 0 40px rgba(0,0,0,0.6),inset 1px 0 0 rgba(201,168,76,0.1)', display: 'flex', flexDirection: 'column' }}>
                        <div style={{ padding: '20px 24px', borderBottom: '1px solid rgba(201,168,76,0.12)', background: 'linear-gradient(135deg,rgba(13,23,42,0.98),rgba(18,32,56,0.98))' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                <div>
                                    ...
                                    <div style={{ fontSize: '12px', fontFamily: 'monospace', color: '#edf2f8', wordBreak: 'break-all' }}>{reasonPopup.entityId}</div>
                                </div>
                                <button type="button" onClick={() => setReasonPopup(null)} style={{ border: '1px solid rgba(201,168,76,0.2)', background: 'rgba(22,33,52,0.9)', borderRadius: '8px', padding: '4px 10px', cursor: 'pointer', fontSize: '14px', color: '#8f9bad', flexShrink: 0, marginLeft: '12px' }}>✕</button>
                            </div>
                            <div style={{ marginTop: '10px', fontSize: '12px', color: '#fbbf24' }}>{reasonPopup.reasons.length} reason{reasonPopup.reasons.length !== 1 ? 's' : ''} detected</div>
                        </div>
                        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 24px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                            {reasonPopup.reasons.length === 0 ? (
                                <div style={{ color: '#4a5568', fontSize: '13px', textAlign: 'center', padding: '40px 0' }}>No reasons recorded</div>
                            ) : reasonPopup.reasons.map((r, i) => (
                                <div key={i} style={{ padding: '12px 16px', borderRadius: '10px', background: 'rgba(217,119,6,0.15)', border: '1px solid rgba(217,119,6,0.3)' }}>
                                    <div style={{ fontSize: '10px', fontWeight: '800', color: '#fbbf24', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '6px' }}>Reason {i + 1}</div>
                                    <div style={{ fontSize: '13px', color: '#edf2f8', lineHeight: 1.6, fontWeight: '500' }}>{humanizeReason([r], reasonPopup.primaryBehavior)}</div>
                                    {humanizeReason([r], reasonPopup.primaryBehavior) !== r && (
                                        <div style={{ fontSize: '11px', color: '#4a5568', marginTop: '6px', fontStyle: 'italic' }}>Raw: {r}</div>
                                    )}
                                </div>
                            ))}
                        </div>
