      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-end justify-between gap-3">
          <div>
            ...
            <div className="mt-1 text-sm text-gray-400">Real-time chain sync, health, and performance.</div>
          </div>
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          <div className="glass p-4">
            ...
            <div className="mt-2 text-2xl font-bold text-gray-100">
              {q.isLoading ? <Skeleton className="h-8 w-16" /> : `${healthyCount} / ${totalCount}`}
            </div>
          </div>
          <div className="glass p-4">
            <div className="flex items-center gap-2 text-xs text-gray-400">
            ...
            <div className="mt-2 text-2xl font-bold text-gray-100">
              {q.isLoading ? (
                ...
                `${perf[perf.length - 1]?.avgLatencyMs ?? 0} ms`
              ) : (
                '—'
              )}
            </div>
          ...
          <div className="glass p-4">
            ...
            <div className="mt-2 text-2xl font-bold text-gray-100">
              {q.isLoading ? (
                ...
                perf[perf.length - 1]?.queueDepth ?? 0
              ) : (
                '0'
              )}
            </div>
            ...
            <div className="mt-3 h-64">
              {q.isLoading ? (
                ...
                <div className="flex h-full items-center justify-center text-sm text-gray-500">
                  Performance data accumulates after first refresh.
                </div>
              ) : (
        ...
        <div className="glass p-4">
          ...
          <div className="mt-1 text-xs text-gray-400">
            Queue a reindex job for a specific address and optional block range.
          </div>
          <div className="mt-4 grid gap-2 sm:grid-cols-[1fr_1fr_auto]">
            <input
              className="input"
              placeholder="Address (0x…)"
              value={reindexAddress}
              onChange={(e) => setReindexAddress(e.target.value)}
            />
            <input
              className="input"
              placeholder="Block range (e.g. 19000000-19010000)"
              value={reindexRange}
              onChange={(e) => setReindexRange(e.target.value)}
            />
            <button
              type="button"
