...
const RECOMMENDATION_CONFIG = {
  ...
  block: {
    icon: ShieldX,
    color: 'text-rose-300',
    bg: 'border-rose-400/20 bg-rose-500/5',
...
export function SimulatorPage() {
  const { register, handleSubmit, formState: { errors } } = useForm<Form>({
    defaultValues: { chain: 'ethereum', amount: '0' },
  })
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data: SimResult) => {
      ...
      if (rec === 'safe') toast.success('Simulation complete — safe')
      else if (rec === 'caution') toast('Simulation complete — caution advised', { icon: '⚠️' })
  ...
  const onSubmit = handleSubmit((f) => {
    if (!f.from.trim() || !f.to.trim()) {
      toast.error('From and To addresses are required')
      return
    }
    mutation.mutate({
      from: f.from.trim(),
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-4 py-6">
      <div className="flex flex-col gap-4">
        <div>
          <div className="text-2xl font-semibold text-gray-100">Transaction simulator</div>
          <div className="mt-1 text-sm text-gray-400">
            Simulate a transaction before sending — get risk analysis, gas estimate, and a safety recommendation.
          </div>
        </div>
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-xl border border-white/10 bg-white/[0.03] p-3">
                    <div className="text-xs text-gray-400">Sender risk</div>
                    <div className={`mt-1 text-2xl font-bold ${result.fromRiskScore >= 60 ? 'text-rose-300' : result.fromRiskScore >= 30 ? 'text-amber-300' : 'text-emerald-300'}`}>
                      {result.fromRiskScore}
                    </div>
                  </div>
                  <div className="rounded-xl border border-white/10 bg-white/[0.03] p-3">
                    <div className="text-xs text-gray-400">Recipient risk</div>
                    <div className={`mt-1 text-2xl font-bold ${result.toRiskScore >= 60 ? 'text-rose-300' : result.toRiskScore >= 30 ? 'text-amber-300' : 'text-emerald-300'}`}>
                      {result.toRiskScore}
                    </div>
                  </div>
                </div>
                ...
                {result.gasEstimate && (
                  <div className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.03] p-3 text-sm">
                    ...
                    <span className="text-gray-300">
                      Gas estimate: <span className="text-gray-100">{result.gasEstimate.toLocaleString()} units</span>
                      {result.gasCostEth !== undefined && (
                        <span className="ml-2 text-gray-400">≈ {result.gasCostEth.toFixed(6)} ETH</span>
                      )}
                    </span>
                  </div>
                )}
                  ...
                  <div className="rounded-xl border border-rose-400/20 bg-rose-500/5 p-3">
                    ...
                    <ul className="space-y-1">
                      {result.riskFlags.map((flag, i) => (
                        <li key={i} className="text-xs text-rose-200/80">• {flag}</li>
                      ))}
                    </ul>
                  </div>
                )}
        ...
        {history.length > 0 && (
          <div className="glass p-4">
            ...
            <div className="mt-3 grid gap-2">
              {history.map((h, i) => {
                const cfg = RECOMMENDATION_CONFIG[h.recommendation]
                const Icon = cfg.icon
                return (
                  <div key={i} className="glass glass-hover flex items-center justify-between gap-3 p-3">
                    ...
                    <div className="flex shrink-0 items-center gap-2">
                      <span className="text-xs text-gray-400">
                        Risk: {Math.max(h.fromRiskScore, h.toRiskScore)}/100
                      </span>
