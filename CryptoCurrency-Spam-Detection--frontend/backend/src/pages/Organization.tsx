...
import { fetchOrganizationKeys, createOrganizationKey, deleteOrganizationKey, revokeOrganizationKey } from '../lib/endpoints'
      ...
      <div className="flex flex-col gap-4">
        ...
        {newKeyValue && (
          <div className="glass border border-emerald-400/20 bg-emerald-500/5 p-4">
            <div className="text-sm font-semibold text-emerald-200">New API key created — copy it now, it won't be shown again</div>
            <div className="mt-2 flex items-center gap-2">
              ...
              <button
                ...
                onClick={() => copyKey('new', newKeyValue)}
              >
                  ...
                  {q.isLoading ? (
                    ...
                    keys.map((k) => (
                      <tr key={k.id} className={['hover:bg-white/[0.02]', !k.isActive ? 'opacity-50' : ''].join(' ')}>
                        ...
                        <td className="px-4 py-3">
                          <span className={`rounded-full border px-2 py-0.5 text-xs ${PERM_COLORS[k.permissions] ?? ''}`}>
                            {k.permissions}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                          <span className={`rounded-full border px-2 py-0.5 text-xs ${TIER_COLORS[k.tier] ?? ''}`}>
                            {k.tier}
                          </span>
                        </td>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1">
                            <button
                              type="button"
                              title="Copy key"
                              className="rounded-lg p-1.5 text-gray-400 hover:bg-white/[0.06] hover:text-gray-100"
                            ...
                            {k.isActive && (
                              <button
                                type="button"
                                title="Revoke key"
                                className="rounded-lg p-1.5 text-gray-400 hover:bg-amber-500/10 hover:text-amber-300"
                            ...
                            <button
                              type="button"
                              title="Delete key"
                              className="rounded-lg p-1.5 text-gray-400 hover:bg-rose-500/10 hover:text-rose-300"
        ...
        <div className="glass p-4">
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            {['address.update', 'alert.triggered', 'risk.change', 'watchlist.match'].map((event) => (
              <label
                key={event}
                className="flex items-center justify-between gap-3 rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm text-gray-200"
              >
                <span>{event}</span>
