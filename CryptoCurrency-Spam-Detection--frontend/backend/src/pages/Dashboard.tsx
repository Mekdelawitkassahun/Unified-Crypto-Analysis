...
export function Dashboard() {
  ...
  const [chain, setChain] = useState<ChainId>("gnosis");
  ...
  const [tab, setTab] = useState<
    "overview" | "analytics" | "dapps" | "anomalies" | "crosschain" | "mev"
  >("overview");
  ...
  useEffect(() => {
    ...
    const ch =
      typeof params.chain === "string" ? (params.chain as ChainId) : null;
    if (addr && ch) {
      setChain(ch);
          ...
          <>
            <div className="glass glass-hover flex flex-col gap-3 p-4 md:flex-row md:items-center md:justify-between">
              <div className="min-w-0">
                ...
                <div className="mt-1 truncate font-mono text-sm text-gray-100">
                  {activeAddress}
                </div>
                <div className="mt-2 flex flex-wrap items-center gap-2">
                  <span className="pill">
                    Chain:{" "}
                    <span className="text-gray-200">{chainMeta.name}</span>
                  </span>
                  {risk ? (
                    <RiskBadge score={risk.score} factors={risk.factors} />
                  ) : null}
