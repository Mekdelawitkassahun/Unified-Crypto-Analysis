...
function mapToCategory(title: string, description: string): string | null {
  ...
  if (text.includes("sanction") || text.includes("ofac")) return "Sanctions";
  ...
  if (
    ...
    return "Stolen funds";
  if (text.includes("darknet") || text.includes("dark web")) return "Darknet";
...
export function RiskBreakdownPanel({
  ...
  const categoryCounts = useMemo(() => {
    ...
    RISK_CATEGORIES.forEach((cat) => (counts[cat] = 0));
              ...
              <tbody className="divide-y divide-white/5">
                {RISK_CATEGORIES.map((cat) => (
                  ...
                  >
                    ...
                    <td
                      className={`px-4 py-2.5 text-right font-mono ${categoryCounts[cat] > 0 ? "text-rose-400 font-bold" : "text-gray-500"}`}
                    >
                      {categoryCounts[cat]}
                    </td>
                  </tr>
              ...
              {factors.length === 0 ? (
                ...
                factors.map((f, idx) => (
                  ...
                  >
                    ...
                    <div
                      className={`shrink-0 rounded-full border px-3 py-1 text-xs ${sevPill(f.severity)}`}
                    >
                      {f.severity}
                    </div>
                  </button>
